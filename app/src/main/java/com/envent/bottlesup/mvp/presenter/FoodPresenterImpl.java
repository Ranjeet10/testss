...
public class FoodPresenterImpl implements MyPresenter.FoodPresenter {
    private MVPView.FoodView view;
    private Call<ResponseBody> foodCall;
    private Call<EventResponse> eventResponseCall;

    @Override
    public void addView(MVPView.FoodView view) {
        this.view = view;
    }
    ...
    @Override
    public void getFoodTabs(int venueId, String apiKey) {
        view.showProgressDialog();
        foodCall = BottlesUpApplication.getApi().getFoodCategoryResponse(String.valueOf(venueId), apiKey);
        foodCall.enqueue(new Callback<ResponseBody>() {
            @Override
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if (o.has(MetaData.MESSAGE.STATUS)) {
                            ...
                            if (status.equals(MetaData.MESSAGE.SUCCESS)) {
                                Gson g = new Gson();
                                view.onFoodTabsResponse(g.fromJson(_res, FoodDrinkTabResponse.class));
                            } else {
